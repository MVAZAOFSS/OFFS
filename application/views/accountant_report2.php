...
<div id="content">
    ...
    <div class="col-lg-9">
           <div class="well well-sm">
            <label class="text text-center text-primary">Frequently report of&nbsp;<span class="badge pull-right">kerosene</span></label>
        </div>
        <div class="col-lg-4">
          ...
          <div id="collapseThree" class="panel-collapse collapse">
            ...
            if($res->num_rows()>0){
                ...
                foreach ($res->result() as $row){
                     echo '<tr>'
                           . '<td>'.$row->sold_date.'</td><td class="pull-right">'.'<a class="btn btn-success btn-xs" onclick="showkerosinehistory(\''.$row->id.'\')"><span class="fa fa-search"></span></a>'.' '.anchor('accountant_controller/reportprint2/'.$row->id,'<button class="btn btn-warning btn-xs"><span class="fa fa-print"></span></button>').'</td>'
                           . '</tr>';
                }
                echo '</tbody>';
                echo '</table>';
    ...
    <script>
      ...
      <script>
    function showkerosinehistory(id){
        $('.petrol').html('Loading...');
        var url="<?php echo site_url('accountant_controller/kerosinereport');?>";
        var url2=url + '/' +id;
