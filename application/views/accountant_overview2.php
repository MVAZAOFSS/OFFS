...
<div id="content">
    <div class="col-lg-9">
        <div class="well well-sm">
            <label class="text text-center text-primary">Present sellers with their respective history&nbsp;<span class="badge pull-right">KEROSENE</span></label>
        </div>
        <div class="col-lg-7">
        <ol class="breadcrumb">
            ...
            <tbody>
                ...
                if(isset($petrol)){
                  foreach($petrol->result() as $row){
                      echo '<tr><td><button class="btn btn-success btn-xs" onclick="kerosinecheck(\''.$row->id.'\')"><span class="fa fa-share-square-o"></span>view</button>'."&nbsp;".anchor('general_accountant/printoverview2/'.$row->id,'<button class="btn btn-warning btn-xs"><span class="fa fa-print"></span>print</button>').'</td>'
    ...
    function kerosinecheck(id){
        $('.kat').html('Loading...');
        var url="<?php echo site_url('general_accountant/kerosine');?>";
        var url2=url + '/' +id;
