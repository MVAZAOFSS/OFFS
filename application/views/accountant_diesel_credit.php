    ...
    <div class="col-lg-9">
      <div class="col-md-5">
              <table class="table table-condensed table-striped" id="record">
                    <thead><tr><th>Diesel creditor(s) List</th></tr></thead>
                    <tbody>
                        <?php
                        $res=$this->db->get_where('tb_diesel',array('status'=>'credit','stat_role'=>$this->session->userdata('role_station')));
                        if($res->num_rows()>0){
                            foreach ($res->result() as $row){
                                echo '<tr><td>'.$row->customer_credit_name.'<button class="btn btn-success btn-xs pull-right"  onclick="creditview(\''.$row->id.'\')">
