   ...
   <ul class=" breadcrumb">
    ...
    <div class="col-lg-9">
          <ul class="list-group"> 
             ...
             <div class="col-lg-6">  
             <?php
             $results=$this->db->get_where('tb_petrol',array('sold_date'=>  date('m/d/Y'),'seller_status'=>'yes','stat_role'=>$this->session->userdata('role_station')));
             if($results->num_rows()>0){
                 echo '<table class="table table-condensed table-striped table-hover">';
                 echo '<thead><tr><th>Date</th><th>Name</th><th class="text-center">Action<b class="caret"></b></th></tr></thead>';
                 foreach ($results->result() as $row){
                 echo '<tbody><tr><td>'.$row->sold_date.'</td><td class="text-primary">'.$row->seller_name.'</td><td> '. '<button class="btn btn-success btn-xs" onclick="petrolcheck(\''.$row->id.'\')" data-target=".display" data-toggle="modal"><span class="fa fa-share-square"></span> view'
                         . '</button>'.'  '.anchor('accountant_controller/sellsprint/'.$row->id,'<button class="btn btn-warning btn-sm"><span class="fa fa-print"> print</span></button>').'</td></tr></tbody>';     
                 }
                  echo '</table>';
                }  else {
           ...
           <div class="input-group">
            <ul class="list-group">
                <li class="list-group-item">Petrol creditor(s) List</li>
            </ul>
            <table class="table table-striped" id="customer">
     <thead><tr><th>Seler name<th>Customer name</th><th>Litre(s)</th><th>Amount to be payed</th><th>Action</th></tr></thead>
                <tbody>
                    <?php 
                    $res=$this->db->get_where('tb_petrol',array('status'=>'credit','stat_role'=>$this->session->userdata('role_station')));
                    if($res->num_rows()>0){
                        foreach ($res->result() as $row){
                            echo '<tr><td>'.$row->seller_name.'</td><td>'.$row->customer_credit_name.'</td><td>'.$row->credit_litre.'</td><td>'.$row->credit_amount.'</td><td><button class="btn btn-success btn-xs" onclick="editprograme(\''.$row->id.'\')" data-target=".displayz" data-toggle="modal"><span class="fa fa-share-square-o">Pay</span></button></td></tr>';
                        }
                    }
                    ?>
                </tbody>
    ...
    function petrolcheck(id){
       var url="<?php echo site_url('accountant_controller/petrolconts');?>";
        var url2=url + '/' +id;
