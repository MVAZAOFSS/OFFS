    ...
    <ul class=" breadcrumb">
        ...
        <div class="col-lg-9">
            <ul class="list-group"> 
             ...
             <div class="col-lg-6">  
             <?php
             $results=$this->db->get_where('tb_kerosine',array('sold_date'=>  date('m-d-Y'),'seller_status'=>'yes','stat_role'=>$this->session->userdata('role_station')));
             if($results->num_rows()>0){
                 echo '<table class="table table-condensed table-striped table-hover">';
                 echo '<thead><tr><th>Date</th><th>Name</th><th class="text-center">Action <b class="caret"></b></th></tr></thead>';
                 foreach ($results->result() as $row){
                 echo '<tbody><tr><td>'.$row->sold_date.'</td><td class="text-primary">'.$row->seller_name.'</td><td> '. '<button class="btn btn-success btn-xs" onclick="kerosinecheck(\''.$row->id.'\')" data-target=".display" data-toggle="modal"><span class="fa fa-share-square"></span> view'
                        . '</button>'.' '.anchor('accountant_controller/kerosinesellsprint/'.$row->id,'<button class="btn btn-warning btn-sm"><span class="fa fa-print"> print</span></button>').'</td></tr></tbody>';     
                 }
                 echo '</table>';
             }  else {
           ...
           <div class="modal fade display" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
            ...
            <ul class="list-group">
 <li class="list-group-item">Kerosene creditors(s) List</li>
            </ul>
    ...
    <script>
    function kerosinecheck(id){
       var url="<?php echo site_url('accountant_controller/kerosineconts');?>";
        var url2=url + '/' +id;
